...
class Config:
    ...
    def is_filesystem_mode_enabled(plugin: Plugin) -> bool:
        ...
        plugin_fs_mode_enabled = get_bool_env(
            f"PODTUBE_FILESYSTEM_MODE_PLUGIN_{plugin.plugin_name}",
            plugin.default_fs_mode_enabled,
        )
        return (
            global_fs_mode_enabled
            and plugin.supports_fs_mode
            and plugin_fs_mode_enabled
        )
