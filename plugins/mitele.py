...
class PluginImpl(Plugin):
    ...
    pattern = r'<script .*>window\.\$REACTBASE_STATE\.container_mtweb = (.*?)(?=</script>)'

    def get_feed(self, feed_id):
        """Calculates and returns the subscribable feed."""
        url = f"https://www.mitele.es/programas-tv/{feed_id}/"
        response = httpx.get(url, follow_redirects=True)
        podcast_dict = self.get_dict(response)
        return PodcastFeed(
            feed_id=feed_id,
            title=podcast_dict['title'],
            description=podcast_dict['title'],
            link=url,
            image=podcast_dict['images']['thumbnail']['src'],
            items=self._get_items(feed_id, podcast_dict["tabs"][0]["contents"][0]["children"]),
        )
    ...
    def get_item_url(self, item_id):
        ...
        return f"https://www.mitele.es/programas-tv/{item_id}/player"
