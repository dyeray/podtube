...
class PluginImpl(Plugin):
    def get_feed(self, feed_id):
        ...
        with httpx.Client() as client:
            ...
            api_response = client.get(
                ...
                headers={
                    'X-IG-App-ID': custom_headers['X-IG-App-ID']
                }
            )
            data = json.loads(api_response.text)
            videos = [item['node'] for item in data['data']['user']['edge_owner_to_timeline_media']['edges'] if item['node']['__typename'] == 'GraphVideo']
        return PodcastFeed(
            feed_id=feed_id,
    ...
    def get_item_url(self, item_id):
        ...
        try:
            ...
            return YoutubeDL().extract_info(self._get_story_url(item_id), download=False)['url']
    ...
    def _get_item(self, item: dict) -> Union[PodcastItem, None]:
        ...
        info = (safe_traverse(item, 'edge_media_to_caption', 'edges', 0, 'node', 'text') or '').split('\n', 1)
        return PodcastItem(
            item_id=item_id,
            title=safe_traverse(info, 0) or '',
            description=safe_traverse(info, 1) or '',
            link=self._get_story_url(item_id),
            date=datetime.datetime.utcfromtimestamp(item['taken_at_timestamp']).replace(tzinfo=datetime.timezone.utc),
